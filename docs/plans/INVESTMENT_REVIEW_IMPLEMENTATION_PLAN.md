...
- 已实现 `portfolio.trade_episode.v1` 的确定性事实投影、稳定身份和 event consumption ledger；
- 已实现 flat/open/add/reduce/close/re-entry，并对 opening balance、转移、修正、公司行动和不可拆分 reversal 保留显式缺口或 blocker；
- 已只读消费 P2B 快照引用，before fallback 保留时间距离，after 仅在 lineage 证明事件已包含时链接；
- 已只消费 `decision_event_links` 的显式 Decision 关系，不按时间或文本推断；
- 已提供本地 JSON artifact 的 build/query/validate 命令，未升级 v2 sidecar schema；
- P2D 未启动，后续需单独 gate 决定 Decision/Information Event 双时间能力。
