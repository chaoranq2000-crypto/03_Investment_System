...
| G1 Evidence Gate | 证据门禁 | evidence manifest 完整，关键证据可定位 | 回到 Evidence Plan |
| G2 Claim Gate | Claim 门禁 | facts / estimates / inferences / comments 分开 | 回到 Claims Draft |
| G3 Report Gate | 报告门禁 | 关键结论有 evidence_id / claim_id / metric_id / TODO | 回到对应报告 skill |
| G4 Exposure Gate | 暴露门禁 | exposure_type、score、confidence、evidence_ids 完整 | 回到 Mapping |
| G5 Backflow Gate | 回写门禁 | 个股发现已回写或明确 no-backflow 理由 | 回到 Backflow |
| G6 Risk Gate | 风险反证门禁 | 风险、反证、missing data 清楚 | 回到报告或 quality-review |
| G7 No Advice Gate | 投资建议门禁 | 无直接买卖建议，无评分替代交易判断 | 修复文本 |
| G8 Close Gate | 收尾门禁 | readout 说明成果、TODO、状态和下一步 | 不能 accepted |
