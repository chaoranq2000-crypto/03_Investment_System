...
> 本文件是 A-share Research OS 的唯一全局 workflow kernel。它定义永久 `workflow_type`、global `stage_id`、global `gate_id`、backflow decision、交接资产和 P2 readiness 条件。
...
|---|---|---|---|
| `segment_to_stock_closed_loop` | 从一个细分、产业链环节、主题或产品开始 | 细分定义 → 证据 → claims/metrics → 公司池 → exposure → 个股样本 → 回写 → 质量审查 | P1/P1.6 主闭环 |
...
| skill | 主要职责 | 不负责 |
|---|---|---|
| `research-orchestrator` | 识别工作流、维护状态、路由 skill、生成 handoff、调度门禁、输出 readout | 不直接写研究结论、不虚构证据、不替代下层 skill |
...
| stage_id | 目标 | 主 skill | 关键输出 | 主要 gate |
|---|---|---|---|---|
| S0 Intake | 明确对象、范围、深度、是否 P2 前置 | `research-orchestrator` | `workflow_state.yaml`、`run_log.md` | G0 |
| S1 Segment Definition | 定义 segment_id、scope_in/out、相邻细分 | `segment-research` | segment definition / boundary note | G0 |
| S2 Evidence Plan & Ingest | 制定证据计划并登记证据 | `evidence-ingest` | raw/processed evidence、`evidence_manifest.csv` | G1 |
| S3 Claims & Metrics Draft | 拆 facts/comments/estimates/inferences 与指标 | `evidence-ingest` + `quality-review` | `claims_draft.csv`、`metrics_draft.csv` | G2, G3 |
| S4 Segment Report Draft | 生成细分研究初稿 | `segment-research` | segment report、evidence map | G4 |
| S5 Company Universe | 找 A 股公司池并区分暴露层级 | `company-universe` | `company_universe.csv` | G5 |
| S6 Exposure Mapping | 建立多对多暴露映射 | `segment-company-mapping` | `segment_company_exposure.csv` | G6 |
| S7 Stock Sample Selection | 选择 1–2 家样本进入个股深度 | `research-orchestrator` | sample selection note | G0, G6 |
| S8 Stock Deep Dive | 生成个股深度样本 | `stock-deep-dive` | stock report、segment_exposure、evidence map | G7 |
| S9 Backflow | 个股发现回写细分、公司池和 exposure | `segment-company-mapping` + `segment-research` | updated exposure / taxonomy candidate / segment TODO | G8 |
| S10 Scorecard & Watchlist Draft | 形成评分和观察项 | `segment-research` + `stock-deep-dive` + `memo-writer` | scorecard、watchlist draft | G9 |
| S11 Quality Review | 总审查 | `quality-review` | quality gate report、quality issues | G1-G9 |
| S12 Fix Loop | 根据质量问题回到对应阶段 | `research-orchestrator` | handoff to target skill | target gate |
| S13 Close Readout | 固化成果、TODO 和状态 | `research-orchestrator` | workflow readout | G10 |
...
| stage_id | 目标 | 主 skill | 关键输出 | 主要 gate |
|---|---|---|---|---|
| T0 Intake | 明确股票、公司、范围、是否已有 linked_segments | `research-orchestrator` | `workflow_state.yaml` | G0 |
| T1 Company Evidence | 收集年报、公告、财务数据、IR、新闻等证据 | `evidence-ingest` | evidence manifest 更新 | G1 |
| T2 Business & Financial Skeleton | 搭业务和财务骨架 | `stock-deep-dive` | business table、financial skeleton | G2, G3 |
| T3 Linked Segment Discovery | 识别公司可能关联的细分 | `stock-deep-dive` + `segment-company-mapping` | linked_segments draft | G6 |
| T4 Segment Context Check | 检查 linked segments 是否已有定义 | `research-orchestrator` | existing links / segment candidates | G0 |
| T5 Mini Segment Card | 对缺失但重要的细分建立最小定义卡 | `segment-research` | segment definition candidate | G4 |
| T6 Exposure Mapping | 更新该公司在多个细分中的 exposure | `segment-company-mapping` | `segment_exposure.yaml`、`segment_company_exposure.csv` | G6 |
| T7 Stock Report Draft | 写个股深度报告 | `stock-deep-dive` | stock deep dive、evidence map | G7 |
| T8 Backflow | 回写 segment taxonomy / company universe / exposure | `segment-company-mapping` + `segment-research` | updated segment assets | G8 |
| T9 Quality Review | 审查个股报告和映射 | `quality-review` | quality report | G1-G9 |
| T10 Close Readout | 输出个股闭环 readout | `research-orchestrator` | workflow readout | G10 |
...
| gate_id | 名称 | 通过条件 | 未通过处理 |
|---|---|---|---|
| G0 Scope Gate | 范围门禁 | workflow_type、对象、深度、out_of_scope 明确 | 回到 Intake。 |
| G1 Evidence Gate | 证据门禁 | evidence manifest 完整，关键证据可定位，缺口显式标注 | 回到 Evidence Plan / Ingest。 |
| G2 Claim Gate | Claim 门禁 | facts / estimates / inferences / management_comment / analyst_view / opinion 分开 | 回到 Claims Draft 或报告修复。 |
| G3 Metric Gate | 指标门禁 | 指标口径、单位、周期、来源、计算方法明确 | 回到 Metrics Draft 或数据层修复。 |
| G4 Segment Report Gate | 细分报告门禁 | 细分定义、产业链、关键结论、风险和 evidence map 可追溯 | 回到 `segment-research`。 |
| G5 Company Universe Gate | 公司池门禁 | 公司池区分真实暴露、候选暴露、叙事暴露和排除理由 | 回到 `company-universe`。 |
| G6 Exposure Gate | 暴露门禁 | exposure_type、score、confidence、evidence_ids、validity 完整 | 回到 `segment-company-mapping`。 |
| G7 Stock Report Gate | 个股报告门禁 | 个股报告可独立阅读，关键结论有 evidence / claim / metric / TODO | 回到 `stock-deep-dive`。 |
| G8 Backflow Gate | 回写门禁 | 个股发现已回写或明确 no-backflow 理由 | 回到 Backflow stage。 |
| G9 No Advice Gate | 投资建议门禁 | 无直接买卖建议，无评分替代交易判断 | 修复文本。 |
| G10 Close Gate | 收尾门禁 | readout 说明成果、TODO、质量状态和下一步 | 不能 accepted。 |
